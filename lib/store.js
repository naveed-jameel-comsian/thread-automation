...
function syncPostingAccounts(entries) {
  for (const entry of entries) {
    const username = typeof entry === 'string' ? entry : entry.username;
    const account = ensurePostingAccount(username);
    if (typeof entry === 'object' && entry !== null) {
      if (typeof entry.postingEnabled === 'boolean') account.postingEnabled = entry.postingEnabled;
      if (typeof entry.paused === 'boolean') account.paused = entry.paused;
    }
  }
  saveState();
}
