 ...
 * Scanner must be closed first — call closeScannerSession() before repost batch.
 */
async function withRepostProfile(username, fn) {
  if (!kameleo.isKameleoEnabled()) {
  ...
  const profile = await kameleo.getProfileByUsername(username);
  if (!profile) {
    throw new Error(`No Kameleo profile named @${username}`);
  }
  if (profile.id === kameleo.SCANNER_PROFILE_ID) {
    throw new Error(`Profile @${username} is the scanner profile — cannot repost from it`);
  }

  await stopKameleoProfile(profile.id);
  const session = await connectKameleoProfile(profile.id);
