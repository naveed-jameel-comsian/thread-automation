...
async function connectRepostProfileSession(username, profileIdHint) {
  const profileId = await resolveRepostProfileId(username, profileIdHint);
  const key = `repost:${username}`;

  if (!sessionCache.has(key)) {
    sessionCache.set(key, (async () => {
      if (!kameleo.isKameleoEnabled()) {
      ...
      return { ...session, profileId, mode: 'kameleo-repost' };
    })());
  }
...
async function openRepostProfileSession(username, profileIdHint) {
  return connectRepostProfileSession(username, profileIdHint);
}

async function closeRepostProfileSession(username) {
  await closeSession(`repost:${username}`);
}
...
async function withRepostProfile(username, fn, profileIdHint) {
  ...
  const context = await openRepostProfileSession(username, profileIdHint);

  try {
    return await fn(context);
  } finally {
    await closeRepostProfileSession(username);
  }
}
